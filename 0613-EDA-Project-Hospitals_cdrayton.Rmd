...
```{r}
#Mean and max of the Hp Knee Procedure
ratings %>%
  group_by(Facility.Type) %>%
...
#scatterplotof heart attack cost of care and the value of heart attack cost
ratings %>%
...
#scatterplotof hip knee cost of care and the value of heart attack cost
ratings %>%
  ggplot(aes(x = `Procedure.Hip Knee.Value`, y = `Procedure.Hip Knee.Cost`,
             color = Facility.Type)) +
  geom_point(alpha = 0.5) +
...
library(usmap)
plot_usmap(data = ratings, values = "mean_cost", color = "red") states = ratings$Facility.State) +
...
#ratings %>%
  ...
  #geom_histogram( y = mean(Rating.Overall)) +
  #theme_bw()
...
#ratings %>%
  #ggplot(aes(x = mean(Rating.Experience),
             #fill = Facility.Type)) +
...
new_var <- rating %>>%
  mutate(
    type = case_when(ratings$`Procedure.Heart Attack.Quality`
      Procedure.Heart Attack.Quality == "Average" ~ 3,
      Procedure.Heart Attack.Quality == "Worse" ~ 2,
      Procedure.Heart Attack.Quality == "Unknown" ~ 1,
      TRUE ~ NA
)
