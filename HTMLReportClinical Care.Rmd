...
- 1. Covid death rate: this is the amount of covid deaths in a particular county divided by the total deaths.

- 2. Vaccination rate: these are different vaccination rates scenarios such as people are vaccinated with one dose and people who are vaccinated with two doses of the Covid-19 vaccine and lastly people who are fully vaccinated with a booster shot.
...
- This plot shows three linear regressions lines corresponding to three different vaccination scenarios in the US.  (Blue shows Vaccinated with only one dose , green represents people who are fully vaccinated with a booster shot and red represents people who are fully vaccinated with two doses of the coronavirus vaccine.
...
gfg_plot <- ggplot(comb_data, aes(y = `COVID-19 Deaths per Capita` )) +  
  ...
  geom_smooth(aes(x = `Vacinated+Booseter`,color = "Vacinated+Booseter"), formula=y~x, method="lm") +
  geom_smooth(aes(x = `Only_One_dose`, color = "Only_One_dose"), formula=y~x, method="lm") +
  geom_smooth(aes(x = `Fully_Vaccinated`, color = "Fully_Vaccinated"), formula=y~x, method="lm") +
  theme_bw() +
...
## Hypothesis - States with higher Unemployment ratings or higher Uninsured people will result
#large percentages of people with poor, or fair health.
...
# Results: Higher Uninsured Ratings does lead to higher Poor or Fair Health Ratings
#but Unemployment does not.
...
##Hypothesis- Less Mental health providers provided in an area will likely to lead to a person
#having more Poor mental health days.
```{r echo = FALSE, warning = FALSE, message = FALSE}
...
## Results - Yes, this shows that less mental providers leads to people having more poor mental health days
...
##Hypothesis - Better income inequality ratios, unemployment rates and high school completion rateswill be association with a lower Years of Potential Life Lost rate but there will be confounding variables that will allow us definitely conclude what variables will lead to decreased YPLL rates per racial group
...
##Visualization of Pivot_Longer [premature_deaths]
...
When looking at unemployment, as the percentage of population 16 and older unemployed increases, it seems that an increase in unemployment rates is assocaited with an increase in the number of years of potential life lost premature deaths increases steadily. Unemployment seems to be an important variable across all races as the premature deaths for each of the five racial groups is dependent on the unemployment variable. 
...
When looking at highschool completion rates, an increase in the percentage of adults 25 and older in a county with high school diplomas seems to be associated with a decrease in years of potential life lost (premature death years).  
...
##Visualization of Pivot_Longer [preventable_stays]
...
##County Plot Preventable Hospital Stays
...
- Despite not having demographic data per input variable (explanatory variables) in our model, there are random forest models per race (Native American, Asian/Pacific Islander, Black, Hispanic, and White). 
...
# Random Forest with Years of Potential Life Lost (premature_deaths) as the predictor variable
```{r echo = FALSE, warning = FALSE, message = FALSE}
...
ggplot(df, aes(predictions, premature_deaths, label = label)) +
  geom_point(alpha = 0.3) +
  geom_abline(intercept = 0, slope = 1) + geom_text_repel() + geom_smooth(method = "lm", se = FALSE) + theme_bw()
```
...
ggplot(df_pop, aes(predictions, premature_deaths, label = labe)) +
  geom_point(alpha = 0.3) +
  geom_abline(intercept = 0, slope = 1) + geom_text_repel() + geom_smooth(method = "lm", se = FALSE) + labs(title ="Outlier Counties in Random Forest Model with County Populations") + theme_bw()

vip(ini_rank_rf, geom = "col") + labs(title = "Variable Importance for Premature Deaths + Population Variable") + theme_bw()
```

## Random Forests by Race
```{r echo = FALSE, warning = FALSE, message = FALSE}
...
ggplot(df3, aes(predictions, premature_deaths, label = label3)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) + geom_text_repel() + geom_smooth(method = "lm") + theme_bw() + labs(x = "Predictions for Years of Potential Life Lost", y = "Years of Potential Life Lost Rate")
...
ggplot(cf_NA, aes(predictions, preventable_stays, label = lab2)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) + geom_text_repel() + geom_smooth(method = "lm") + theme_bw() + labs(x = "Predictions for Preventable Hospital Stays", y = "Preventable Hospital Stays")
...
- A Poisson regression to predict Covid deaths and counties with high Covid transmission Rates

Access to Care:

- The Data Analysis for Access to Care can lead to practical applications like where to map new facilities that assist health like for mental health. 
